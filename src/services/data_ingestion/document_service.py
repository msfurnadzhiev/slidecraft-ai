...
class DocumentService:
    ...
    def _chunks_processing(self, document_content: DocumentContent) -> int:
        ...
        for idx, text in enumerate(stripped_texts):
            if text:
                log.debug("Embedding content for page %d.", idx + 1)
                content_vectors.append(self.embedder.generate_embedding(text))
            else:
                log.debug("Page %d is empty – using zero vector for content.", idx + 1)
                content_vectors.append(_ZERO_VECTOR)

            summary_text = summaries[idx]
            if summary_text:
                log.debug("Embedding summary for page %d.", idx + 1)
                summary_vectors.append(self.embedder.generate_embedding(summary_text))
            else:
                log.debug("Page %d has no summary – using zero vector.", idx + 1)
                summary_vectors.append(_ZERO_VECTOR)
